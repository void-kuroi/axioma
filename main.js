document.addEventListener("DOMContentLoaded", () => {
            ...
            if (isTabActive && meshActive) {
                ...
                for (let i = 0; i < particles.length; i++) {
                    ...
                    for (let j = i; j < particles.length; j++) {
                        ...
                        if (dist < 120) {
                            ctx.beginPath(); ctx.strokeStyle = `rgba(139, 141, 145, ${0.6 - dist/120})`; 
    ...
    // MATRIX RAIN Y SCRAMBLE
    const mCanvas = document.getElementById('matrix-canvas');
    let mCtx = null; let mRainActive = false; let mDrops = [];
    ...
    let termStep = 0; let userData = { nombre: '', asunto: '', mensaje: '' };
    ...
    if (btnTerminal && termOverlay) {
        ...
        termInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && termInput.value.trim() !== '') {
                ...
                if (val.toLowerCase() === '/help') { termOutput.innerHTML += `<br><br>> COMANDOS DISPONIBLES:<br>> /matrix - Inicia Lluvia Roja.<br>> /stop - Detiene Matrix.<br>> /clear - Limpia la consola.<br>> /exit - Cierra el Terminal.`;
                } else if (val.toLowerCase() === '/clear') { termOutput.innerHTML = `> CONSOLA LIMPIADA.<br>> SISTEMA AXIOMA LISTO.`; termStep = 3; 
                } else if (val.toLowerCase() === '/exit') { termClose.click();
                } else if (val.toLowerCase() === '/matrix') { termOutput.innerHTML += `<br><br>> EJECUTANDO PROTOCOLO_RED_RAIN... [OK]`; if(mCanvas) { mRainActive = true; meshActive = false; mCanvas.style.display = 'block'; systemHalted = true; if(sysStatusText) sysStatusText.innerText = '[ SYS_OVERRIDE: MATRIX ]'; animateMatrix(); }
                } else if (val.toLowerCase() === '/stop') { termOutput.innerHTML += `<br><br>> DETENIENDO PROTOCOLO_RED_RAIN... [OK]`; mRainActive = false; meshActive = true; if(mCanvas) mCanvas.style.display = 'none';
                } else if (termStep === 0) { userData.nombre = val; termOutput.innerHTML += `<br><br>> VERIFICADO: ${userData.nombre}.<br>> DEFINA VECTOR DE ASUNTO:`; termStep++;
                } else if (termStep === 1) { userData.asunto = val; termOutput.innerHTML += `<br><br>> ASUNTO REGISTRADO.<br>> INSERTE DATOS (Mensaje):`; termStep++;
                } else if (termStep === 2) { userData.mensaje = val; termOutput.innerHTML += `<br><br>> ENCRIPTANDO PAQUETE DE DATOS...<br>> TRANSMITIENDO A SERVIDOR CENTRAL...`; setTimeout(() => { termOutput.innerHTML += `<br><br><span style="color:#00FF00">> PAQUETE ENVIADO CON ÉXITO. FIN DE LA CONEXIÓN.</span>`; playSound('click'); setTimeout(() => { termOverlay.style.display = 'none'; }, 2000); }, 1500); }
                termOverlay.querySelector('.terminal-window').scrollTop = termOverlay.querySelector('.terminal-window').scrollHeight;
            }
        });
    }

    // PRELOADER (Intro SIEMPRE ACTIVA)
    ...
    if (preloader) { setTimeout(typeBootSequence, 500); }
    ...
    // NAVEGACIÓN Y UX PRINCIPAL
    function autoScroll() {
        if (Math.ceil(window.innerHeight + window.scrollY) >= document.documentElement.scrollHeight - 2) { systemHalted = true; if(sysStatusText) { sysStatusText.innerText = '[ EXEC_COMPLETE ]'; sysStatusText.style.color = 'var(--gris-codigo)'; } }
    ...
    function iniciarSistema() {
        ...
        window.addEventListener('scroll', () => { if(progressEl) progressEl.style.height = `${(document.documentElement.scrollTop / (document.documentElement.scrollHeight - document.documentElement.clientHeight)) * 100}%`; });

        const sections = document.querySelectorAll('section'); const navDots = document.querySelectorAll('.radar-dot');
        window.addEventListener('scroll', () => { 
            let current = ''; sections.forEach(sec => { if (pageYOffset >= sec.offsetTop - 200) current = sec.getAttribute('id'); }); 
            navDots.forEach(dot => { dot.classList.remove('active'); if (dot.getAttribute('href').includes(current)) dot.classList.add('active'); }); 
        });

        const grain = document.getElementById('grain-filter'); let grainTimer;
        window.addEventListener('scroll', () => { if(grain) { grain.style.opacity = '0.08'; clearTimeout(grainTimer); grainTimer = setTimeout(() => { grain.style.opacity = '0.04'; }, 150); } });
        ...
        document.querySelectorAll('.text-stop, p, h1, h2, h3, h4').forEach(el => {
            el.addEventListener('click', () => { if(!systemHalted) { document.body.classList.add('flash-override'); playSound('click'); setTimeout(() => document.body.classList.remove('flash-override'), 100); } systemHalted = true; if(sysStatusText) { sysStatusText.innerText = '[ SYS_MANUAL_OVERRIDE ]'; sysStatusText.style.color = 'var(--gris-codigo)'; } });
        });
