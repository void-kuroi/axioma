/**
 * =======================================================================
 * AXIOMA | CORE SYSTEM LOGIC (BLINDADO ANTI-CRASH)
 * =======================================================================
 */
...
function typeBootSequence() { 
    if (bootIdx < bootLines.length && termText) { 
        termText.innerHTML += bootLines[bootIdx] + "<br>"; bootIdx++; setTimeout(typeBootSequence, 400); 
    } else if (startBtn) { 
...
try { hasVisited = localStorage.getItem('axioma_visted_today') === 'true'; } catch(e) { console.warn("Memoria bloqueada en modo local."); }

if (hasVisited && preloader) {
    preloader.style.display = 'none';
    iniciarSistema();
    if(sysStatusText) { sysStatusText.innerText = '[ RECONEXIÓN RÁPIDA ESTABLECIDA ]'; sysStatusText.style.color = 'var(--gris-codigo)'; }
} else {
...
if(startBtn && preloader) {
    startBtn.addEventListener('click', () => {
        ...
        try { localStorage.setItem('axioma_visted_today', 'true'); } catch(e) {}
...
function autoScroll() {
    if (Math.ceil(window.innerHeight + window.scrollY) >= document.documentElement.scrollHeight - 2) { 
        systemHalted = true; if(sysStatusText) { sysStatusText.innerText = '[ EXEC_COMPLETE ]'; sysStatusText.style.color = 'var(--gris-codigo)'; }
    }
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyHistory.join('') === secretCode.join('')) { document.body.classList.toggle('god-mode'); playSound('click'); if(sysStatusText) sysStatusText.innerText = '[ MODO DIOS: ACTIVADO ]'; }
});

const ctxMenu = document.getElementById('context-menu');
document.addEventListener('contextmenu', (e) => { e.preventDefault(); if(ctxMenu) { ctxMenu.style.display = 'flex'; ctxMenu.style.left = e.clientX + 'px'; ctxMenu.style.top = e.clientY + 'px'; playSound('hover'); }});
document.addEventListener('click', (e) => { if(ctxMenu && !ctxMenu.contains(e.target)) ctxMenu.style.display = 'none'; });
const ctxCopy = document.querySelector('.ctx-copy'); if(ctxCopy) ctxCopy.addEventListener('click', () => { navigator.clipboard.writeText(window.location.href); ctxMenu.style.display = 'none'; });
const ctxAudio = document.querySelector('.ctx-audio'); if(ctxAudio && btnAudioToggle) ctxAudio.addEventListener('click', () => { btnAudioToggle.click(); ctxMenu.style.display = 'none'; });
const ctxReload = document.querySelector('.ctx-reload'); if(ctxReload) ctxReload.addEventListener('click', () => { location.reload(); });
