/**
 * =======================================================================
 * AXIOMA CORE | v1.0.0 | CORE SYSTEM LOGIC (RESTAURACIÓN NÚCLEO)
 * =======================================================================
 */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // =========================================
    // 1. ESTADO GLOBAL Y SELECTORES UNIFICADOS
    // =========================================
    ...
    // =========================================
    // 2. MOTOR DEL CURSOR CUSTOM (Rojo Fijo Brutalista)
    // =========================================
    ...
    if (!isMobile && cursorX) {
        document.addEventListener('mousemove', (e) => { 
            ...
            // Calculamos velocidad para la estela táctica
    ...
    function createGhost() {
        ...
        if(ghostContainer) ghostContainer.appendChild(ghost);
        
        // Eliminamos la estela táctica tras la animación
    ...
    // =========================================
    // 5. MATRIX RAIN NATIVA Y CIFRADO SCRAMBLE
    // =========================================
    ...
    // =========================================
    // 7. ARRANQUE DEL SISTEMA (BOOT SEQUENCE SIEMPRE ACTIVA)
    // =========================================
    const preloader = document.getElementById('preloader'); 
    const termText = document.getElementById('term-text'); 
    const startBtn = document.getElementById('start-btn');
    const bootLines = ["> CONECTANDO CON SERVIDOR CENTRAL...", "> VERIFICANDO MANIFIESTO_NÚCLEO... [OK]", "> COMPILANDO LÓGICA_ MODULAR... [OK]", "> INICIALIZANDO MOTOR_RENDERING_3D... [OK]", "> SISTEMA AXIOMA ENGINE LISTO."];
    let bootIdx = 0;
    ...
    // Animación sin condicionales de memoria local
    if (preloader) {
    ...
    if(startBtn && preloader) {
        startBtn.addEventListener('click', () => {
            ...
            // Blindaje técnico: Guardamos memoria local SÓLO después del clic, no antes.
            try { localStorage.setItem('axioma_engine_visited', 'true'); } catch(e) {}
            setTimeout(() => { preloader.style.display = 'none'; iniciarSistema(); }, 500);
        });
    }

    // =========================================
    // 8. LÓGICA DE UX PRINCIPAL (Auto-scrolling Brutalista)
    // =========================================
    function autoScroll() {
    ...
    function iniciarSistema() {
        if(sysStatusText) { sysStatusText.innerText = `> AXIOMA_HARDWARE_ENGINE v1.0 ONLINE.\n> ESPERANDO INTERACCIÓN...`; sysStatusText.style.color = 'var(--blanco-hueso)'; }
        ...
        // Blindaje técnico: Arrancar el motor de auto-scrolling
    ...
    // =========================================
    // 9. EASTER EGG REINSTALADO
    // =========================================
    let secret = "axioma"; let inputStr = "";
    document.addEventListener("keydown", (e) => {
        inputStr += e.key.toLowerCase();
        if (inputStr.slice(-secret.length) === secret) {
            initAudio(); playSound('click'); 
            document.body.classList.add('god-mode'); playSound('matrix'); 
            secret = "off"; inputStr = "";
        } else if (secret === "off" && inputStr.slice(-secret.length) === secret) {
            document.body.classList.remove('god-mode'); playSound('matrix'); 
            secret = "axioma"; inputStr = "";
        }
        if (inputStr.length > 20) inputStr = inputStr.slice(-10);
    });
